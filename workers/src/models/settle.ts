...
export async function settleMatches(env: Env): Promise<{ settled: number; eloUpdated: number; scores: number }> {
  ...
  for (const m of matches ?? []) {
    ...
    if (pred) {
      ...
      // 只記錄「有真實 TW/Pinnacle 賠率」的場次。
      // 賠率用「開賽前收盤」（最貼近實際下注價、優先台灣運彩）算損益；CLV = 早盤 vs 收盤線位移。
      // 沒有真實盤 → 不記入戰績，但仍更新 Elo。
...
export async function entryClosingOdds(env: Env, matchId: string, sel: string, kickoff: string): Promise<{ entry: number | null; closing: number | null }> {
